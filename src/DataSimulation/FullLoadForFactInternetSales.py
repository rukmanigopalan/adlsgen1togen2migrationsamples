...
for startdate in reversed(range(StartDate,EndDate)):
  for batch in range(0,1):
    ...
    events = spark.read.option("header", "true").csv("/"+file).drop('_c0')
    ...
    events.coalesce(1).write.csv(path=csv_location, mode="append", header="true")
...
# MAGIC %fs rm -r /mnt/AdventureWorksProd/Raw/FactInternetSales/2019/
